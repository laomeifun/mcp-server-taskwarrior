...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: "get_next_tasks",
        description: "Get a list of all pending tasks (returns JSON with UUIDs)",
  ...
  try {
    ...
    switch (name) {
      ...
      case "mark_task_done": {
        ...
        const content = execSync(`task ${parsed.data.uuid} done`, { maxBuffer: 1024 * 1024 * 10 }).toString().trim();
        return {
          content: [{ type: "text", text: content }],
        };
      }
